...
export default function Home() {
  ...
  const getImageUrl = (item: GalleryItem) => {
    ...
    // Handle Payload media relation object
    if (item.image && typeof item.image === 'object') {
      ...
      // If it's a relation object with nested url
      if ((item.image as any).sizes?.card?.url) return (item.image as any).sizes.card.url
      if ((item.image as any).sizes?.thumbnail?.url) return (item.image as any).sizes.thumbnail.url
    }
